...
3. **Invariant Preservation**: Generic invariant preservation theorem
   (`generic_invariant_preservation`) plus specific invariants for valid
   status, separation, and auditability.

4. **Modal Links** (in `EpArch.ModalLinks` namespace):
   Safety ↔ V-independence, Sensitivity ↔ E-coverage. Both modal
   epistemology conditions reduce to header preservation.
...
- Trace metrics and convergence (~lines 998–1120)
- Step-preserved invariants (~lines 1124–1455)
...
/-- Localization score: how precisely a challenge identifies the problem.

    All challenges are field-specific (score = 1) by construction. -/
def localization_score (_c : EpArch.Challenge PropLike Reason Evidence) : Nat := 1

/-- Headers improve localization score.

    Challenges against deposits with headers have optimal localization. -/
theorem headers_improve_localization
    (s : SystemState PropLike Standard ErrorModel Provenance)
    (d_idx : Nat)
    (_h_header : depositHasHeader s d_idx)
    (c : EpArch.Challenge PropLike Reason Evidence) :
    localization_score c = 1 := by
  rfl
...
/-! =====================================================================
    Modal Links — Safety and Sensitivity
    =====================================================================

    This section connects modal epistemology (Safety, Sensitivity) to the
    S/E/V field structure, using the AdversarialBase.lean threat model.

    - Safety (wouldn't falsely believe) ↔ V-independence from luck
    - Sensitivity (would notice if false) ↔ E-field covers counterfactual

    The AdversarialBase.lean model provides the bridge: V-spoofing attacks
    violate Safety, E-field gaps violate Sensitivity.

    Note: header_preserved is opaque, so we define modal conditions in
    terms of it. The concrete model provides witnesses with specific values.
-/

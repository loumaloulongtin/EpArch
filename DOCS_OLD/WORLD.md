...
The "all three" requirement only appears at the top-level convergence theorem `world_assumptions_force_bank_primitives`, which needs all three because `containsBankPrimitives` requires *all eight* architectural features and three of those eight are each individually grounded by one bundle. Remove any one bundle and the corresponding feature loses its forcing argument — the theorem would have a gap. The assumptions are not discharged by the conditional — they are its antecedents. They are discharged only in `kernel_world_forces_bank_primitives`, which instantiates the concrete `WitnessCtx` witness and closes the statement with zero free hypotheses.
